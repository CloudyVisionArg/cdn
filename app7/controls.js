...
function getInputText(pId, pLabel, pValue) {
    ...
    }).append(pLabel + '_hola').appendTo($itemInner);
...
function getAutocomplete(pId, pLabel, pSource, pMultiple) {
    ...
    if (pMultiple) {
        ...
        $li.find('.input-clear-button').click(function (e) { $acEl.val('').focus() });
    ...
    */
    ...
    function getNavbarButton(pButton) {
        if (pButton == 'new')
            ...
            return getLink({ text: 'Guardar' });
        else if (pButton == 'exit')
            return getLink({ text: 'Salir' });
