...
function pageInit(e, page) {
    ...
    Object.assign(pageEl.drs, {
        ...
        folder, folder2, $navbar, import: importProp, f7Page, actionsPopup,
    });
    pageEl.crm = pageEl.drs; // bg compat
}
...
async function loadViewSection(pContainer, pCallback) {
    ...
    try {
        ...
        if (view.Definition.Groups.Items.length < level) {
            ...
            if (res.length == 0) {
                ...
                $li = $('<li/>', {
                    ...
                    style: `background-color: var(${ document._f7version >= 8 ? '--f7-list-group-title-bg-color' : '--f7-list-item-divider-bg-color' });`,
                }).appendTo($ul);
        ...
        console.error(err);
        pContainer.html(`<div class="block-title">Error</div><div class="block">${ errMsg(err) }</div>`); 
